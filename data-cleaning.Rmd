...
info <- read_sheet_clean(ss_url, "info") |>
  select(-c(virement, lien_forms:code_forms)) |>
...
```{r clean data srss}
# alainst_louis@hotmail.com couche parfois en am 
srss_clean <- merge(srss, info, by = "courriel") |>
  # correct date for euro subjects
  mutate(
    horodatage = if_else(
      ...
      as_datetime(as.numeric(horodatage), origin = "1970-01-01")
    ),
    # keep only date part of horodaatge to prevent conversion day for euro subjects
  ...
  mutate(
    ...
    sleep_time = ifelse(bed_time <= wake_up_time,
...
# Apply the function for both load_rpe and load_rpe_velo
srss_wellness_training <- merge(srss_wellness, srss_training_day, by = c("date", "nom"), all = TRUE) %>%
  group_by(nom) %>%
  fill(c(courriel, hrv_android_minus_1d, ath_id, api_key), .direction = "up") %>%
  ungroup() %>%
  filter(n() > 1) %>% 
