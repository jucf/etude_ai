---
title: "Rapport d'analyse hebdomadaire du projet de recherche L-Tips-UQTR"
date: 
dev: cairo_pdf
df_print: paged
output:
...
params:
   athlete_selected: "Mick Déry"
   selection_debut: "2024-05-02"
   selection_fin: "2024-05-29"
header-includes:
- \usepackage{geometry}
- \geometry{left=0.45in, right=0.45in, top=0.65in, bottom=0.50in} # Adjust margins as needed
- \usepackage{makecell}
- \usepackage{booktabs}
- \usepackage{colortbl}
...
- \usepackage{float}
- \pagestyle{fancy}
- \rhead{\includegraphics[height=1cm]{a-ltips.png}}
...
**Semaine :** Lundi `r (as.Date(params$selection_fin) - days(5))` (identifié par les \colorbox{lightgray}{zones ombragées})
...
```{r globale, fig.width=11, fig.height=13.5,  fig.cap="Évolution de la charge d’entraînement et des mesures descriptives (voir page suivante pour les détails)."}
#charge
gg1 <- ggplot(data_filt %>% filter(!is.na(load_rpe)), aes(date, weight = load_rpe / 1000)) +
  annotate(
    "rect",
    xmin = as.Date(as.Date(params$selection_fin) - days(5)), 
    xmax = as.Date(as.Date(params$selection_fin)),
    ymin = -Inf, ymax = Inf,
    color = NA, alpha = 0.075
  ) +
...
```{r recup, fig.width=11, fig.height=13.5, fig.cap="Analyse des variations de l'état de récupération (voir page suivante pour les détails)."}
...
```{r stress, fig.width=11, fig.height=13.5, fig.cap="Analyse des variations de l'état de stress (voir page suivante pour les détails)."}
gg_stress <-
...
gg_sommeil <-
  ...
  geom_hline(yintercept = mean(data_filt$sleep_quality, na.rm = TRUE), 
             linetype = "dotted",  # Specify linetype directly
             color = "grey",
             size = 1.5
  ) +
  geom_bar(fill = "#E0EEEE", width = 0.5) +
  ...
  theme(
    ...
    panel.grid.minor.x = element_blank(),
    legend.margin = margin(t = 0, unit = 'cm'),
    plot.margin = margin(0.10, 1.75, 0.10, 0.10, "cm")  # Adjusting plot margins to create space for the annotation outside the plot area
  ) +
  guides(color = guide_legend(order = 1),
         fill = guide_legend(order = 2)) +
  annotate(
    "text",
    x = max(df_am_pm$date),
    y = as.numeric(format(round(mean(data_filt$sleep_quality, na.rm = TRUE), digits = 2), nsmall = 2)),
    label = paste0(format(round(mean(data_filt$sleep_quality, na.rm = TRUE), digits = 2), nsmall = 2)),       
    color = "black",
    hjust = -1.5,
    vjust = 0,
    size = 5,
  ...
  "text",
  x = max(df_am_pm$date),
  y = round(mean(data_filt$sleep_quality, na.rm = TRUE), digits = 2),
  label = "Moy",
  color = "grey",
  hjust = -1.5,
  vjust = 1.5,
...
gg_temps_sommeil <-
  ...
  geom_hline(yintercept = round(mean(data_filt$sleep_time, na.rm = TRUE), digits = 1),
             linetype = "dotted",  # Specify linetype directly
             color = "grey",
             size = 1.5
  ) +
  ...
  annotate(
    "text",
    x = max(df_am_pm$date),
    y = as.numeric(format(round(mean(data_filt$sleep_time, na.rm = TRUE), digits = 2), nsmall = 2)),
    label = paste0(format(round(mean(data_filt$sleep_time, na.rm = TRUE), digits = 2), nsmall = 2)),
    color = "black",
    hjust = -1.5,
    vjust = 0,
    size = 5,
  ...
  "text",
  x = max(df_am_pm$date),
  y =  round(mean(data_filt$sleep_time, na.rm = TRUE), digits = 1),
  label = "Moy",
  color = "grey",
  hjust = -1.5,
  vjust = 1.5,
...
gg_heure_couche <-
  ...
  annotate(
    "rect",
    xmin = max(as.Date(df_am_pm$date, format = "%Y-%m-%d") - days(5)),
    xmax = max(as.Date(df_am_pm$date, format = "%Y-%m-%d")),
    ymin = round_date(min(df_am_pm$graph_dttm_bed - 5400), "1 hours"),
    ymax = round_date(max(df_am_pm$graph_dttm_bed + 5400), "1 hours"), 
    color = NA, alpha = 0.075
  ) +
   geom_hline(yintercept = mean(df_am_pm$graph_dttm_bed),
             linetype = "dotted",  # Specify linetype directly
             color = "grey",
             size = 1.5
  ) +
  ...
  theme(
    ...
    plot.title = element_text(margin = margin(b = -10)),
    legend.margin = margin(t = 0, unit = 'cm'),
    plot.margin = margin(0.10, 1.75, 0.10, 0.10, "cm")  
  ) +
  annotate(
    "text",
    x = max(as.Date(df_am_pm$date, format = "%Y-%m-%d") -0.55),
    y = mean(df_am_pm$graph_dttm_bed),
    label = format(mean(df_am_pm$graph_dttm_bed), "%H:%M"),
    color = "black",
    hjust = -1.5,
    vjust = 0,
    size = 5,
  ...
  annotate(
  "text",
  x = max(as.Date(df_am_pm$date, format = "%Y-%m-%d")),
  y = mean(df_am_pm$graph_dttm_bed),
  label = "Moy",
  color = "grey",
  hjust = -1.5,
...
gg_heure_leve <-
  ...
  annotate(
    ...
    xmin = max(as.Date(df_am_pm$date, format = "%Y-%m-%d") - days(5)),
    xmax = max(as.Date(df_am_pm$date, format = "%Y-%m-%d")),
    ymin = round_date(min(df_am_pm$graph_dttm_wake - 3600), "1 hours"),
    ymax = round_date(max(df_am_pm$graph_dttm_wake + 3600), "1 hours"), 
    color = NA, alpha = 0.075
  ) +
  geom_hline(yintercept = mean(df_am_pm$graph_dttm_wake),
             linetype = "dotted",  # Specify linetype directly
             color = "grey",
             size = 1.5
  ) +
  ...
  annotate(
    "text",
    x = max(as.Date(df_am_pm$date, format = "%Y-%m-%d") -0.55),
    y = mean(df_am_pm$graph_dttm_wake),
    label = format(mean(df_am_pm$graph_dttm_wake), "%H:%M"),
    color = "black",
    hjust = -1.5,
    vjust = 0,
    size = 5,
...
gg_charge_sommeil <-
  ...
  geom_hline(yintercept = mean(data_filt$sleep_load, na.rm = TRUE), 
             linetype = "dotted",  # Specify linetype directly
             color = "grey",
             size = 1.5
  ) +
  ...
  annotate(
    "text",
    x = max(df_am_pm$date),
    y =  as.numeric(format(round(mean(data_filt$sleep_load, na.rm = TRUE), digits = 1), nsmall = 1)),
    label = paste0(format(round(mean(data_filt$sleep_load, na.rm = TRUE), digits = 1), nsmall = 1)),
    color = "black",
    hjust = -1.5,
    vjust = 0,
    size = 5,
  ...
  "text",
  x = max(df_am_pm$date),
  y = mean(data_filt$sleep_load, na.rm = TRUE),
  label = "Moy",
  color = "grey",
  hjust = -1.5,
  vjust = 1.5,
...
graphs <- grid.arrange(
  ...
  heights = c(1.65, 1.65, 2.30, 2.30, 1.60)
) 
```
